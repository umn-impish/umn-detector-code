...
class X123Health(ctypes.Structure):
    ...
    def to_json(self):
        ...
        converters = {"board_temp": c_to_k, "det_high_voltage": lambda x: 0.5 * x}
        return {
            k: {
                "value": converters.get(k, lambda x: x)(getattr(self, k)),
                "unit": units[k],
...
class X123NominalSpectrumStatus:
    def __init__(
        self, timestamp_seconds: int, count_histogram: list[int], status: bytes
    ):
...
class X123Debug:
    ...
    def decode(self) -> dict[str, object]:
        ...
        """
        TYPE_MAP = ["histogram", "diagnostic", "ascii-settings"]
        DECODE_MAP = [
        ...
        return {"type": TYPE_MAP[self.type], "data": DECODE_MAP[self.type]()}
    ...
    def _decode_histogram(self):
        ...
        for i in range(0, len(data), 3):
            histogram.append(data[i] | (data[i + 1] << 8) | (data[i + 2] << 16))
