...
def decode_health():
    p = argparse.ArgumentParser(description="Decode health files to JSON")
    p.add_argument("health_files", nargs="+", help="files to decode to JSON")
...
def decode_x123_sci():
    p = argparse.ArgumentParser(description="Decode X123 science files to JSON")
    p.add_argument("x123_files", nargs="+", help="gzipped files to decode to JSON")
    p.add_argument("output_fn", help="output file name to write JSON")
    args = p.parse_args()
...
def decode_hafx_debug_hist():
    ...
    out = {"histograms": [d["registers"] for d in decoded]}
    with open(args.output_fn, "w") as f:
...
def collapse_health(dat: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    ret |= {"timestamp": [d["timestamp"] for d in dat]}
    return ret
...
def decode_exact_sci():
    ...
    """
    p = argparse.ArgumentParser(description="Decode EXACT science files to JSON")
    p.add_argument("files", nargs="+", help="files to decode to JSON")
    p.add_argument("output_fn", help="output file name to write JSON")
    args = p.parse_args()
